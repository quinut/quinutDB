...
export function rowToFrontendItem(row: any): FrontendItem {
  return {
    ...
    ratings: {
      adoption: row.baseline_adoption ?? 3,
      easeOfUse: row.baseline_ease_of_use ?? 3,
      activity: row.baseline_activity ?? 3,
    },
...
export function itemToDbRow(item: any, type: 'frontend' | 'cfw') {
  return {
    ...
    baseline_adoption: item.ratings?.adoption ?? 3,
    baseline_ease_of_use: item.ratings?.easeOfUse ?? 3,
    baseline_activity: item.ratings?.activity ?? 3,
